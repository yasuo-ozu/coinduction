        ...
        for graph in &mut updated_graphs {
            ...
            if let Some(target_node) = graph.find_constraint(target_constraint) {
                ...
                for additional_constraint in &args.additional_constraints {
                    let adapted_constraint =
                        adapt_constraint_types(additional_constraint, target_constraint);
...
fn adapt_constraint_types(
    ...
    if let (syn::Type::Path(constraint_path), syn::Type::Path(target_path)) =
        (&constraint.ty, &target_constraint.ty)
    {
        // Build substitution map for type parameters
        extract_type_substitutions(
            &constraint_path.path,
            &target_path.path,
            &mut substitution_map,
        );
    }
